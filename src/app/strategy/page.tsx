...
export default function StrategyPage() {
    ...
    const { seed, completeStep, budget: contextBudget, setBudget: setContextBudget, setMarket: setContextMarket } = useGravity();
    ...
    // Initialize local state from context if available
    useEffect(() => {
        if (contextBudget && contextBudget > 0) {
            setLocalBudget(contextBudget.toString());
        }
    }, [contextBudget]);
