...
export default function TimelinePage() {
    ...
    const handleGenerate = async (e: React.FormEvent) => {
        ...
        // Validate date range: year must be >= current year and <= current year + 3
        const currentYear = new Date().getFullYear();
        const selectedYear = new Date(startDate).getFullYear();

        if (selectedYear < currentYear || selectedYear > currentYear + 3) {
            const errorMsg = lang === "es"
                ? `🚫 Error: El año debe estar entre ${currentYear} y ${currentYear + 3}.`
                : lang === "ca"
                    ? `🚫 Error: L'any ha d'estar entre ${currentYear} i ${currentYear + 3}.`
                    : `🚫 Error: Year must be between ${currentYear} and ${currentYear + 3}.`;
            setError(errorMsg);
            return;
        }
